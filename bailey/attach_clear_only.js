...
// Fetch students from Supabase (only those with clear=true)
async function getStudentsByClasses(classIds) {
  const { data, error } = await supabase
    .from("students")
    .select("studentid, name, fathername, mobilenumber, class_id, Clear")
    .in("class_id", classIds)
    .eq("Clear", true); // only parents with clear=true
...
(async function main() {
  ...
  if (!recipients.length) {
    console.log("⚠️ No valid WhatsApp numbers with clear=true. Exiting.");
    rl.close();
    return;
  }
